...
// Copyright 2003 - 2011 (c) George Petrov, Patrick Woldberg, www.DMXzone.com
//
// Version: 2.1.12
// ------------------------------------------------------------------------------
...
class pureFileUpload
{
  ...
  function error($error, $extra="") {
    ...
    // File required
    case 'required':
      echo "<b>File is required!</b><br/><br/>";
      echo "There is no file uploaded or file is 0 bytes.<br/>";
      break;
    }
  ...
  function doUpload() {
    ...
    $this->debug("nameToLower(<font color=\"#990000\">".$this->nameToLower."</font>)<br/>");
	$this->debug("requireUpload(<font color=\"#990000\">".$this->requireUpload."</font><br/>");
	...
	if ($this->requireUpload) {
	  foreach ($_FILES as $field => $value) {
		  if ($_FILES[$field]['name'] == '' || $_FILES[$field]['size'] == 0) {
			  $this->error("required", "");
		  }
	  }
	}
    ...
    if ($this->redirectURL <> '' && !isset($_POST['FlashUpload'])) {
      ...
      header( "Location: %s", $this->redirectURL));
    }
  }
