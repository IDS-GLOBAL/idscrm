...
echo $found_app_deal_number = $row_show_vehicle['vthubmnail_file_path'];
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_qryDeal =  "SELECT * FROM `idsids_idsdms`.`deals_bydealer`, `idsids_idsdms`.`customers` WHERE `deal_dealerID` = '$did' AND `deal_number` = '$colname_qryDeal' AND `deals_bydealer`.`customer_id` = `customers`.`customer_id`";
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_salespersons = "SELECT * FROM `idsids_idsdms`.`sales_person` WHERE `salesperson_id` = '$sid'";
