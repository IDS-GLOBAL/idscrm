...
function convertdlrtask($milliseconds) {
    ...
    $formatted = date('Y, n, d, h, i', $milliseconds);
    ...
    return $formatted;
		...
		$query_dlr_tasks = "SELECT * FROM  `idsids_idsdms`.`dealers_tasks` WHERE `dealers_tasks`.`task_did` = '$did'";
...
return $milliseconds;
		...
		mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
		$query_dealerappts = "SELECT * FROM  `idsids_idsdms`.`dealers_appointments` WHERE `dealers_appointments`.`dlr_appt_did` = '$did' AND `dealers_appointments`.`ids_sys_owned` IS NULL";
		$find_dealerappts = mysqli_query($idsconnection_mysqli, $query_dealerappts);
		$row_find_dealerappts = mysqli_fetch_assoc($find_dealerappts);
...
$count_totalRows_find_dealerappts++;
			...
			//if(!$appt_url_goto){}
			$appt_url_goto = 'appointment.php?appt_token='.$dlr_appt_token;
 ...
 } while ($row_find_dealerappts = mysqli_fetch_assoc($find_dealerappts)); 
...
return $milliseconds;
		...
		mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
		$query_sys_appts = "SELECT * FROM `idsids_idsdms`.`dealers_appointments` WHERE `dealers_appointments`.`dlr_appt_did` = '$did' AND `dealers_appointments`.`ids_sys_owned` IS NOT NULL";
...
$count_totalRows_find_sys_appts++;
			...
			$appt_url_goto = "appointment.php?appt_toke=".$dlr_appt_token;
 ...
 } while ($row_find_sys_appts = mysqli_fetch_assoc($find_sys_appts)); 
...
";
		...
		mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
		$query_sys_appts = "SELECT * FROM `idsids_idsdms`.`dealers_tasks` WHERE `dealers_tasks`.`task_did` = '$did' AND `dealers_tasks`.`task_completed` IS NOT NULL";
		$find_sys_appts = mysqli_query($idsconnection_mysqli, $query_sys_appts);
		$row_find_sys_appts = mysqli_fetch_assoc($find_sys_appts);
		$totalRows_find_sys_appts = mysqli_num_rows($find_sys_appts);
...
$count_totalRows_find_sys_appts++;
			...
			$task_id = $row_find_sys_appts['task_id'];
			$task_title = $row_find_sys_appts['task_title'];
			$taskstart_unixtime = $row_find_sys_appts['taskstart_unixtime'];
			$taskend_unixtime = $row_find_sys_appts['taskend_unixtime'];
			//$task_url_goto = $row_find_sys_appts['appt_url_goto'];
			$task_token = $row_find_sys_appts['task_token'];
			$task_className = 'idsRobot';
			
			//if(!$task_url_goto){$task_url_goto = "taskview.php?task_token=$task_token";}

			$task_url_goto = "taskview.php?task_token=$task_token";
				  ...
				  echo "start: '$taskstart_unixtime',"."\n";
				  echo "end: '$taskend_unixtime',"."\n";
				  echo "allDay: false,"."\n";
				  echo "color: '#F21D1D',"."\n";
				  echo "className: '$task_className',"."\n";
				  echo "overlap: true,"."\n";
				  echo "backgroundColor: '#17F06C',"."\n";
				  echo "url: '$task_url_goto'"."\n";
