...
var lat = $('input#dlr_geo_latitude').val();
var long  = $('input#dlr_geo_longitude').val();
...
$(document).ready(function() {
		  ...
		  function processJSON(json) {
			...
			if(json.status == 'OK'){
						...
						console.log('County Or State: '+listing_county);
						if(listing_county){
							//$('select#state').val(listing_county);
							// Sometimes County Sometimes State
						}
					...
					alert('There Was A Problem With The Address To Geo Decode Sorry Try Again!!!');
						...
						$.getScript('//maps.google.com/maps/api/js?key=AIzaSyBbYYY54ZOHlFuHK7v-Fq3kWQYgwim8xec&sensor=false&callback=initialize');		
		...
		$("button#pull_dlr_map").on('click', function(event) {
				...
				if(!dlr_geo_latitude && !dlr_geo_longitude){
					
					console.log('dlr_geo_longitude: Empty && dlr_geo_latitude');
					
					
				}else{

					console.log('dlr_geo_longitude: Full && dlr_geo_latitude');

									if(dlr_ok_googlemp == 'OK'){
										console.log('Canceling == OK');
										$.getScript('//maps.google.com/maps/api/js?key=AIzaSyBbYYY54ZOHlFuHK7v-Fq3kWQYgwim8xec&sensor=false&callback=initialize');		
...
string = 'Status: <span id="map_status">' + status + '</span><br />Address: <span id="address">'+ address +'</span><br /><br />Latitude: <span id="new_latitude">'+lat+'</span> <br />Longitude: <span id="new_longitude">' + long + '</span>';
				...
				$.getScript('http://maps.google.com/maps/api/js?key=AIzaSyCHtiyM7Z3wOJ34Qp-BoFIZzcvVOILPfPo&sensor=false&callback=initialize');				
