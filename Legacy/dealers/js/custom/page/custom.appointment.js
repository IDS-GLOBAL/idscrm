...
var again =	$('#datetimepicker12').data("DateTimePicker").date();
		var dodate = moment(again, "YYYY-MM-DD HH:mm:ss a" );
		console.log('dodate: '+dodate.format('YYYY-MM-DD HH:mm:ss a'));
		console.log('human read: '+dodate.format('MM/DD/YYYY HH:mm:ss a'));
		var dlr_appt_starttime_humanread = dodate.format('MM/DD/YYYY HH:mm a');
...
$('button#switch_appointment_repair_shops').click(function(){
		...
		$('button#saveOldAppointment').click( function(){
			...
			if(!dlr_appt_id){
				...
				alert('Sorry This appointment Cannot Be Saved');
			...
			var dlr_appt_starttime_humanread = $('input#dlr_appt_starttime_humanread').val();
			var dlr_appt_endtime_humanread = $('input#dlr_appt_endtime_humanread').val();
			...
			var slct_appointment_snooze = document.getElementById("appointment_snooze");
			var appointment_snooze =  slct_appointment_snooze.options[slct_appointment_snooze.selectedIndex].value;
		...
		$('select#appointment_snooze').bind('click change', function(){
						process_appointmentimeview();				  
		});
...
//debugger;
			...
			if(!dlr_appt_starttime_humanread){
			var well =	$('#datetimepicker12').data("DateTimePicker").date();
			console.log('well: '+well);
			var startdate = moment(well, "YYYY-MM-DD HH:mm:ss a" );
			var startdate2 = startdate.format('YYYY-MM-DD HH:mm:ss a');
			...
			$('input#dlr_appt_startunixtime').val(startdate2);
				...
				// var returned_endate = moment(enddate).add(2, 'hours');				

				var slct_appointment_snooze = document.getElementById("appointment_snooze");
				var howlong =  slct_appointment_snooze.options[slct_appointment_snooze.selectedIndex].value;
				...
				var dlr_appt_endtime_humanread = moment(enddate).add(howlong, howlong_type).format('MM/DD/YYYY h:mm a')
