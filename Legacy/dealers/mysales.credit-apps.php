...
$query_find_creditapps = "SELECT * FROM `credit_app_fullblown` WHERE `credit_app_fullblown`.`applicant_sid` = '$sid' AND `credit_app_fullblown`.`applicant_did` = '$did' OR `credit_app_fullblown`.`applicant_sid2` = '$sid' AND `credit_app_fullblown`.`applicant_did` = '$did' ORDER BY `credit_app_fullblown`.`credit_app_fullblown_id` DESC";
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_next_appnumber = "SELECT * FROM `idsids_idsdms`.`credit_app_fullblown` WHERE `credit_app_fullblown`.`applicant_did` = '$creditapp_did' ORDER BY `app_number` DESC LIMIT 1";
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <tbody>
                      <?php if($totalRows_find_creditapps != 0): do { ?>
                            <?php
								...
								if(!$app_number){
								 ...
								 write_app_number($credit_app_id, $applicant_did, $app_number);
								...
								$credit_app_link = "mysales.credit-app.joint.php?app_id=$credit_app_id&keytoken=$applicant_app_token&app_token2=$applicant_app_joint_token";

								$print_credit_app_link = "mysales.autoloanCreditApp.joint-pdf.php?credit_app_id=$credit_app_id&keytoken=$applicant_app_token&app_token2=$applicant_app_joint_token";



								}else{

								$credit_app_link = "mysales.credit-app.php?app_id=$credit_app_id&keytoken=$applicant_app_token";

								$print_credit_app_link = "mysales.autoloanCreditApp-pdf.php?credit_app_id=$credit_app_id&keytoken=$applicant_app_token";
					...
					$created_at = $row_find_creditapps['application_created_date'];
                      ...
                      <tr id="<?php echo $row_find_creditapps['credit_app_fullblown_id']; ?>" class="">
                        ...
                        <?php } while ($row_find_creditapps = mysqli_fetch_assoc($find_creditapps)); endif; ?>
                    </tbody>
                    <tfoot>
                    <tr>
                        <th>Received When</th>
