...
$query_find_leads = "SELECT * FROM `cust_leads` WHERE  `cust_leads`.`cust_salesperson_id` = '$sid' AND `cust_dealer_id` = '$did' OR `cust_salesperson2_id` = '$sid' AND `cust_dealer_id` = '$did' ORDER BY `cust_leadid` DESC";
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_find_thisunreadlead = "SELECT * FROM `cust_leads` WHERE  `cust_leads`.`cust_salesperson_id` = '$sid' AND `cust_dealer_id` = '$did AND `cust_leads`.`cust_leadid` = '$lead_id'  OR `cust_salesperson2_id` = '$sid' AND `cust_dealer_id` = '$did AND `cust_leads`.`cust_leadid` = '$lead_id' LIMIT 1";
...
<?php do { ?>
	...
	$print_link = "print.mysales.lead.pdf.php?leadid=".$cust_leadid;
	...
	?>
                    ...
                    <tr class="gradeX <?php if($cust_seenbydlr == '0'){ echo 'unseen'; }else{ echo 'seen'; } ?>">
                        <td>
                        <a href="mysales.lead.view.php?leadid=<?php echo $cust_leadid; ?>" target="_parent">
                        	<?php 
                        ...
                        <td>
                        	<a href="mysales.lead.view.php?leadid=<?php echo $cust_leadid; ?>" target="_blank">
