...
$query_view_thiscustomer =  "SELECT * FROM `idsids_idsdms`.`customers` WHERE `customers`.`customer_id` = '$colname_view_thiscustomer'";
...
$customer_testimony_id = $customer_token_id = mysql_real_escape_string(trim($row_view_thiscustomer['customer_testimony_id'])); 
...
$customer_home_state = mysql_real_escape_string(trim($row_view_thiscustomer['customer_home_state'])); 
$customer_home_zip = mysql_real_escape_string(trim($row_view_thiscustomer['customer_home_zip'])); 
...
$customer_desired_mo_payment = mysql_real_escape_string(trim($row_view_thiscustomer['customer_desired_mo_payment'])); 
...
$row_find_sales_person_name = mysqli_fetch_array($find_sales_person_name);
$totalRows_find_sales_person_name = mysqli_num_rows($find_sales_person_name);



$sales_person_name = $row_find_sales_person_name['salesperson_firstname'].' '.$row_find_sales_person_name['salesperson_lastname'];
...
$row_howmany_creditapps = mysqli_fetch_array($howmany_creditapps);
...
if($totalRows_howmany_creditapps == 0 || !$app_number){$app_number = '1000'; }else{ $app_number = 1+$app_number; }
 ...
 $create_conversion_customer_to_credit_app_sql = "
INSERT INTO `idsids_idsdms`.`credit_app_fullblown` SET
				...
				`applicant_sid_name` = '$sales_person_name',
				...
				`applicant_desired_mo_payment` = '$customer_desired_mo_payment'
";
