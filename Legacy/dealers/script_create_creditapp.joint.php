...
$cust_vehicle_id = mysqli_real_escape_string($idsconnection_mysqli, trim($_POST['vehicle_id']));
...
// This Selects the next credit app number that belongs to dealer to keep in order.
...
//Removed $applicant_app_token because we are going to create another credit application with a new token
//echo 'script_create_creditapp.joint.php'.'<br />';

$create_credit_app_fullblown_id_sql = "
INSERT INTO `idsids_idsdms`.`credit_app_fullblown` SET
	`applicant_did` = '$did',
	`applicant_vid` = '$cust_vehicle_id',
	`applicant_app_token` = '$token',
	`joint_or_invidividual` = '1',
	`applicant_app_joint_token` = '$applicant_app_token',
