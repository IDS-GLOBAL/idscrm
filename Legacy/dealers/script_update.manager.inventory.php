...
$query_found_vehicle = "SELECT * FROM `idsids_idsdms`.`vehicles` WHERE `vehicles`.`vid` = '$vehicle_id'";
...
$result_update_vehicle_sql = mysqli_query($idsconnection_mysqli, $update_vehicle_sql);
		echo "<script> window.location.replace('manager.inventory.php?vstat=$vlivestatus'); </script>";
