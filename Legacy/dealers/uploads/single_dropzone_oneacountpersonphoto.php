...
if(!$_GET['account_person_id']) exit;

if(!$_FILES) exit;

//print_r($_FILES);
if (isset($_FILES['file']['name'], $_GET['did'], $_GET['account_person_id'], $_GET['changeprofile_final'])) {
...
$acid_photo_team_id = mysql_real_escape_string(trim($_GET['acid_photo_team_id']));
$account_person_id = mysql_real_escape_string(trim($_GET['account_person_id']));

$acid_photo_date_taken = mysql_real_escape_string(trim($_GET['acid_photo_datetaken']));
$acid_photo_albumcomments = mysql_real_escape_string(trim($_GET['acid_photo_albumcomments']));
$changeprofile_final = mysql_real_escape_string(trim($_GET['changeprofile_final']));

mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_find_acid_profile_photos = "SELECT * FROM `idsids_idsdms`.`account_person_photos` WHERE account_person_photos.acid_photo_did = '$did' AND account_person_photos.acid_photo_account_person_id = '$account_person_id' ORDER BY account_person_photos.acid_photo_sort_orderno DESC";
$find_acid_profile_photos = mysqli_query($idsconnection_mysqli, $query_find_acid_profile_photos);
$row_find_acid_profile_photos = mysqli_fetch_assoc($find_acid_profile_photos);
$totalRows_find_acid_profile_photos = mysqli_num_rows($find_acid_profile_photos);

$acid_photo_sort_orderno = $totalRows_find_acid_profile_photos++;
	...
	// Make Team Folder
	if (!file_exists("../../vehicles/photos/$did/media/acid$account_person_id")) {
		mkdir("../../vehicles/photos/$did/media/acid$account_person_id", 0777, true);
		//echo 'Made Team Folder';
	}
	
	if(!file_exists("../../vehicles/photos/$did/media/acid$account_person_id/thumbs")){
		mkdir("../../vehicles/photos/$did/media/acid$account_person_id/thumbs", 0777, true);
	}
	...
	$photo_file_path = "../../vehicles/photos/$did/media/acid$account_person_id/$fileupload";
	
	$photo_thumb_fpath = "../../vehicles/photos/$did/media/acid$account_person_id/thumbs/$fileupload";
...
$acid_photo_token = createAquickkToken();


$acid_photo_album_token = substr(md5(rand(0,200)),0,20);
...
//$acid_photo_datetaken = get_Datetime_Now();
				...
				$acid_photo_file_path = str_replace("../../", "../", $photo_file_path);
				
				$acid_photo_thumb_fpath = str_replace("../../", "../", $photo_thumb_fpath);
				
                //echo $photo_file_path;
				echo $open_url = "https://images.autocitymag.com/"."$did/media/acid$account_person_id/thumbs/$fileupload";
				
				$log .= '<a hrer="'.$open_url.'">'.$open_url.'</a>';

				$insert_acidphoto_sql = "
				INSERT INTO	`idsids_idsdms`.`account_person_photos` SET
						`acid_photo_did` = '$did',
						`acid_photo_account_person_id` = '$account_person_id',
						`acid_photo_team_id` = '$acid_photo_team_id',
						`acid_photo_status` = '1',
						`acid_photo_likes` = '0',
						`acid_photo_abuses` = '0',
						`acid_photo_byip` = '$ip',
						`acid_photo_bymobile` = '$mobiledevice',
						`acid_photo_bybrowser` = '$browser',
						`acid_photo_geo_latitude` = '',
						`acid_photo_geo_longitude` = '',
						`acid_photo_album_token` = '$acid_photo_album_token',
						`acid_photo_albumname` = 'Profile',
						`acid_photo_albumcomments` = '$acid_photo_albumcomments',
						`acid_photo_datetaken` = '$acid_photo_date_taken',
						`acid_photo_sort_orderno` = '$acid_photo_sort_orderno',
						`acid_photo_added_bywho` = 'Dealer Account',
						`acid_photo_token` = '$acid_photo_token',
						`acid_photo_file_name` = '$fileupload',
						`acid_photo_open_url` = '$open_url',
						`acid_photo_file_path` = '$acid_photo_file_path',
						`acid_photo_file_width` = '$vphotowidth',
						`acid_photo_file_height` = '$vphotoheight',	
						`acid_photo_thumb_fname` = '$fileupload',
						`acid_photo_thumb_fpath` = '$acid_photo_thumb_fpath',
						`acid_photo_caption` = '$acid_photo_albumcomments'
				";

	$result_acidphoto_sql = mysqli_query($idsconnection_mysqli, $insert_acidphoto_sql);
...
*/
	...
	if($changeprofile_final == 1){
		...
		$save_account_person_photo_url_sql = "UPDATE `idsids_idsdms`.`account_person` SET
									`profile_image` = '$acid_photo_file_path'
								WHERE
