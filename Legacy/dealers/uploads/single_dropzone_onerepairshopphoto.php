...
if(!$_GET['salesperson_id']) exit;

if(!$_FILES) exit;

//print_r($_FILES);
if (isset($_FILES['file']['name'], $_GET['did'], $_GET['salesperson_id'], $_GET['changeprofile_final'])) {
...
$sid_photo_team_id = mysql_real_escape_string(trim($_GET['sid_photo_team_id']));
$salesperson_id = mysql_real_escape_string(trim($_GET['salesperson_id']));

$sid_photo_date_taken = mysql_real_escape_string(trim($_GET['sid_photo_datetaken']));
$sid_photo_albumcomments = mysql_real_escape_string(trim($_GET['sid_photo_albumcomments']));
$changeprofile_final = mysql_real_escape_string(trim($_GET['changeprofile_final']));

mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_find_team_profile_photos = "SELECT * FROM `idsids_idsdms`.`sales_person_photos` WHERE sales_person_photos.sid_photo_did = '$did' AND sales_person_photos.sid_photo_sid = '$salesperson_id' ORDER BY sales_person_photos.sid_photo_sort_orderno DESC";
...
$sid_photo_sort_orderno = $totalRows_find_team_profile_photos++;
	...
	// Make Team Folder
	if (!file_exists("../../vehicles/photos/$thisdid/media/sid$salesperson_id")) {
		mkdir("../../vehicles/photos/$did/media/sid$salesperson_id", 0777, true);
		echo 'Made Team Folder';
	}
	
	if(!file_exists("../../vehicles/photos/$thisdid/media/sid$salesperson_id/thumbs")){
		mkdir("../../vehicles/photos/$thisdid/media/sid$salesperson_id/thumbs", 0777, true);
	}
	...
	$photo_file_path = "../../vehicles/photos/$thisdid/media/sid$salesperson_id/$fileupload";
	
	$photo_thumb_fpath = "../../vehicles/photos/$thisdid/media/sid$salesperson_id/thumbs/$fileupload";
...
//$sid_photo_datetaken = get_Datetime_Now();
				...
				echo $open_url = "https://images.autocitymag.com/"."$thisdid/media/sid$salesperson_id/thumbs/$fileupload";
				
				$log .= '<a hrer="'.$open_url.'">'.$open_url.'</a>';

				$insert_teamphoto_sql = "
				INSERT INTO	`idsids_idsdms`.`sales_person_photos` SET
						`sid_photo_did` = '$did',
						`sid_photo_sid` = '$salesperson_id',
						`sid_photo_team_id` = '$sid_photo_team_id',
						`sid_photo_status` = '1',
						`sid_photo_likes` = '0',
						`sid_photo_abuses` = '0',
						`sid_photo_byip` = '$ip',
						`sid_photo_bymobile` = '$mobiledevice',
						`sid_photo_bybrowser` = '$browser',
						`sid_photo_geo_latitude` = '',
						`sid_photo_geo_longitude` = '',
						`sid_photo_album_token` = '$sid_photo_album_token',
						`sid_photo_albumname` = 'Profile',
						`sid_photo_albumcomments` = '$sid_photo_albumcomments',
						`sid_photo_datetaken` = '$sid_photo_date_taken',
						`sid_photo_sort_orderno` = '$sid_photo_sort_orderno',
						`sid_photo_added_bywho` = 'Dealer Account',
						`sid_photo_token` = '$sid_photo_token',
						`sid_photo_file_name` = '$fileupload',
						`sid_photo_open_url` = '$open_url',
						`sid_photo_file_path` = '$sid_photo_file_path',
						`sid_photo_file_width` = '$vphotowidth',
						`sid_photo_file_height` = '$vphotoheight',	
						`sid_photo_thumb_fname` = '$fileupload',
						`sid_photo_thumb_fpath` = '$sid_photo_thumb_fpath',
						`sid_photo_caption` = '$sid_photo_albumcomments'
				";
	...
	if($changeprofile_final == 1){
		...
		$save_salesperson_photo_url_sql = "UPDATE `idsids_idsdms`.`sales_person` SET
									`profile_image` = '$sid_photo_file_path'
								WHERE
