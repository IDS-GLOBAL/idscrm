...
if(!$_GET['team_id']) exit;

if(!$_FILES) exit;

//print_r($_FILES);
if (isset($_FILES['file']['name'], $_GET['did'], $_GET['team_id'], $_GET['changeprofile_final'])) {
...
$team_id = mysql_real_escape_string(trim($_GET['team_id']));

$team_photo_date_taken = mysql_real_escape_string($_GET['team_photo_date_taken']);
$team_photo_albumcomments = mysql_real_escape_string(trim($_GET['team_photo_albumcomments']));
...
$team_photo_sort_orderno = $totalRows_find_team_profile_photos++;
	...
	// Make Team Folder
	if (!file_exists("../../vehicles/photos/$thisdid/media/team$team_id")) {
		mkdir("../../vehicles/photos/$did/media/team$team_id", 0777, true);
		echo 'Made Team Folder';
	}
	
	if(!file_exists("../../vehicles/photos/$thisdid/media/team$team_id/thumbs")){
		mkdir("../../vehicles/photos/$thisdid/media/team$team_id/thumbs", 0777, true);
	}
	...
	$photo_file_path = "../../vehicles/photos/$thisdid/media/team$team_id/$fileupload";
	
	$photo_thumb_fpath = "../../vehicles/photos/$thisdid/media/team$team_id/thumbs/$fileupload";
...
$team_photo_token = createAquickkToken();


$team_photo_album_token = substr(md5(rand(0,200)),0,20);
...
//$team_photo_datetaken = get_Datetime_Now();
				...
				$team_photo_file_path = str_replace("../../", "../", $photo_file_path);
				
				$team_photo_thumb_fpath = str_replace("../../", "../", $photo_thumb_fpath);
				
                //echo $photo_file_path;
				echo $open_url = "https://images.autocitymag.com/"."$thisdid/media/team$team_id/thumbs/$fileupload";

				$insert_teamphoto_sql = "
				INSERT INTO	`idsids_idsdms`.`dealers_teams_photos` SET
						`team_photo_did` = '$did',
						`team_photo_team_id` = '$team_id',
						`team_photo_status` = '1',
						`team_photo_likes` = '0',
						`team_photo_abuses` = '0',
						`team_photo_byip` = '$ip',
						`team_photo_bymobile` = '$mobiledevice',
						`team_photo_bybrowser` = '$browser',
						`team_photo_geo_latitude` = '',
						`team_photo_longitude` = '',
						`team_photo_album_token` = '$team_photo_album_token',
						`team_photo_albumname` = 'Profile',
						`team_photo_albumcomments` = '$team_photo_albumcomments',
						`team_photo_datetaken` = '$team_photo_date_taken',
						`team_photo_sort_orderno` = '$team_photo_sort_orderno',
						`team_photo_bywho` = 'Dealer Account',
						`team_photo_token` = '$team_photo_token',
						`team_photo_file_name` = '$fileupload',
						`team_photo_open_url` = '$open_url',
						`team_photo_file_path` = '$team_photo_file_path',
						`team_photo_width` = '$vphotowidth',
						`team_photo_height` = '$vphotoheight',	
						`team_photo_thumb_fname` = '$fileupload',
						`team_photo_thumb_fpath` = '$team_photo_thumb_fpath',
						`team_photo_caption` = '$team_photo_albumcomments'
				";
	...
	if($changeprofile_final == 1){
		...
		$save_dealer_teams_photo_url_sql = "UPDATE `idsids_idsdms`.`dealers_teams` SET
									`dlr_team_photo_url` = '$team_photo_file_path',
									`dlr_team_photo_filename` = '$fileupload',
									`dlr_team_photo_open_url` = '$open_url'
								WHERE
									`dlr_team_id` = '$team_id'
									";
		$result_save_dealer_teams_photo_url_sql = mysqli_query($idsconnection_mysqli, $save_dealer_teams_photo_url_sql);
