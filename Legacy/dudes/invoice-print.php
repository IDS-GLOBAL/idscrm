...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_qryInvoice =  "SELECT * FROM `idsids_idsdms`.`ids_toinvoices` WHERE `ids_toinvoices`.`invoice_id` = '$colname_qryInvoice'";
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_queryInvoice =  "SELECT * FROM `idsids_idsdms`.`ids_toinvoices` WHERE `ids_toinvoices`.`invoice_id` = '$colname_queryInvoice'";
...
$did = $row_qryInvoice['invoice_dealerid']; //Hackishere

$invoice_tokenid = $row_qryInvoice['invoice_tokenid'];

$colname_dlrSlctBySsnDid = "-1";
if (isset($_GET['dealer_id'])) {
  $colname_dlrSlctBySsnDid = $_GET['dealer_id'];
}
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_dlrSlctBySsnDid =  "SELECT * FROM `idsids_idsdms`.`dealers` WHERE `dealers`.`id` = '$colname_dlrSlctBySsnDid'";;
...
$dudesid = $row_userDudes['dudes_id']; //$row_qryInvoice['salesPerson1ID'];
$dudesid2 = $row_userDudes['dudes_id']; //$row_qryInvoice['salesPerson1ID'];

mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_last_invoicenumberdid = "SELECT * FROM `idsids_idsdms`.`ids_toinvoices` WHERE `ids_toinvoices`.`invoice_dealerid` = `invoice_dealerid` ORDER BY `invoice_number` ASC";
...
include("../dealer/definitions-invoice.php");
...
$appid = $colname_qryInvoice;
	...
	//include("../dealer/_defintions-deal-applicant.php");
...
//include('../dealer/definitions-deal-buyersguide.php');


//include('../dealer/bg-calculating.php');


//require('../dealer/functions_deal-pdf-inc.php');


require('../dealer/fpdf-Invoice.php');
//require('../dealer/fpdf.php');
...
function Footer()
{
    ...
    $this->Cell(80,10,"IDSCRM.com, LLC",0,0,'R');
}
}
...
$pdf->Cell(120,6,"IDSCRM.COM, LLC", 0,0,'L');
...
*/
	...
	$pdf->Cell(110,5,"309 WINTHROP LN", 0,1,'L');
	//$pdf->Cell(110,5,"Suite D, 227", 0,1,'L');
	$pdf->Cell(190,5,"McDonough, GA    30253", 0,4,'L');
