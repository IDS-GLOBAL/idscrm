...
$(document).ready(function () {
	...
	$("input#vstockno").on('keyup', function (){

		var vstockno = $("input#vstockno").val().replace(/,/g, '');
		var thisdid = $("input#thisdid").val().replace(/,/g, '');

		$.post('ajax/checkvstockno.php', {vstockno: vstockno, did: thisdid }, 
			   
			   function(result) {
				$('div#vstocknofeedback').html(result).show();
			   });
...
function addcartoinventory(){
		...
		var vin_make = $('input#vin_make').val();
		...
		if(vstockno == empty) { alert('Please Enter Stock Number'); return false; }
		
		var vinstockpassornot = $("input#vinstockpassornot").val();
		if(vinstockpassornot == 0){ alert('Sorry Duplicate Stock Numbers Are Not Allowed'); return false; }
	   ...
	   function(result) {
		$('div#createVehicleResult').html(result).hide();
		//passYesYearvin();
	   });
...
function run_matrix(){
		...
		$('input#vspecialprice').val(vspecialprice);
 		...
 		console.log(vspecialprice.toFixed(2));
