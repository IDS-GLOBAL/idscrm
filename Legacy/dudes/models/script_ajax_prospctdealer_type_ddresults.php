...
if(isset($_POST['prospect_cities'], $_POST['prospect_states'])):
	...
	if($_POST['prospect_cities'] != 'ALL'){
		
		$end_prospect_statesql = "`dealer_prospects`.`state` = '$prospect_states' ";
	}else{
		$end_prospect_statesql = "";
		}
	
	
	if($_POST['prospect_states'] != 'ALL'){
			$end_prospect_citysql = " AND `dealer_prospects`.`city` = '$prospect_cities' ";
		}else{
			
			$end_prospect_citysql = "";
		}
...
$query_distnct_dlrprspct_dtypes = "
SELECT  count( DISTINCT `dtype_id` ) AS `total_city_records`, 
`dealer_prospects`.`city`, `dudes_dealer_types`.`dtype_id`, 
`dudes_dealer_types`.`dtype_label` 
FROM  `idsids_tracking_idsvehicles`.`dudes_dealer_types`
