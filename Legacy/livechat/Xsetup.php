...
// <!-- upon new release: add the corrisponding version upgrade lines here -->
         ...
         if($CSLH_Config['version'] == "4.0.5"){ $installationtype = "upgrade405"; }     
         
         if($CSLH_Config['version'] == "5.0.0"){ $installationtype = "upgrade500"; }   
         if($CSLH_Config['version'] == "5.0.1"){ $installationtype = "upgrade501"; }   
         if($CSLH_Config['version'] == "5.0.2"){ $installationtype = "upgrade502"; } 
         if($CSLH_Config['version'] == "5.0.3"){ $installationtype = "upgrade503"; } 
         if($CSLH_Config['version'] == "5.0.4"){ $installationtype = "upgrade504"; }                                              
         if($CSLH_Config['version'] == "5.0.5"){ $installationtype = "upgrade505"; }             
         ...
         if( (empty($installationtype)) && (empty($UNTRUSTED['freshinstall'])) ){
         	 ...
         	 print " Please visit the website craftysyntax.com and download the latest release OR ask Crafty Syntax ";
         	 print " by emailing <b>csyntax@gmail.com</b> why this install will not upgrade from version to <b>". $CSLH_Config['version'] ."</b> to $version ";
...
if ($UNTRUSTED['action'] == "INSTALL"){
 ...
 if($errors == ""){
   ...
   if($installationtype == "upgrade22"){
     ...
     mysqli_query($idsconnection_mysqli, $sql,$conn);
   ...
   if($installationtype == "upgrade25"){
       ...
       ";
 		$results = mysqli_query($idsconnection_mysqli, $sql,$conn);
		if(!$results) 
		{
       ...
       $results = mysqli_query($idsconnection_mysqli, "INSERT INTO `livehelp_modules` (id,name,path) VALUES (1, 'Live Help!', 'livehelp.php')",$conn);
       $results = mysqli_query($idsconnection_mysqli, "INSERT INTO `livehelp_modules` (id,name,path) VALUES (2, 'Contact', 'leavemessage.php')",$conn);
       $results = mysqli_query($idsconnection_mysqli, "INSERT INTO `livehelp_modules` (id,name,path,adminpath) VALUES (3, 'Q & A', 'user_qa.php','qa.php')",$conn);
       $results = mysqli_query($idsconnection_mysqli, "INSERT INTO `livehelp_modules_dep` VALUES (1, 1, 1, 1, 'N', '')",$conn);
       $results = mysqli_query($idsconnection_mysqli, "INSERT INTO `livehelp_modules_dep` VALUES (2, 1, 2, 2, 'N', 'Y')",$conn);
    ...
    if($installationtype == "upgrade26"){
     ...
     if( ($installationtype == "upgrade271") || 
        ...
        $sql = "ALTER TABLE `livehelp_messages` ADD `typeof` VARCHAR(30) NOT NULL ";
         ...
         $result = mysqli_query($idsconnection_mysqli, $sql,$conn);
...
if( ($installationtype == "upgrade2125" ) || ($installationtype == "upgrade2126" ) || ($installationtype == "upgrade2127") ){           	 
 ...
 if( ($installationtype == "upgrade2169")  ){
  ...
  if( ($installationtype == "upgrade323") || ($installationtype == "upgrade324") || ($installationtype == "upgrade325") ) { 
    ...
    $res = mysqli_query($idsconnection_mysqli, $sql,$conn);  
...
"CREATE TABLE `livehelp_config` (
  `version` varchar(10) NOT NULL default '3.4.6',
  ...
  PRIMARY KEY  (`version`)
		...
		if(!$results) 
		{
 		...
 		$results = mysqli_query($idsconnection_mysqli, $insert_query,$conn);
		if(!$results) 
		{
			echo "<H2>Query went bad!</H2>\n";
			echo mysql_errno().":  ".mysql_error()."<P>";
...
// default stuff...  				        
    $sql = "UPDATE `livehelp_config` SET chatcolors='fefdcd,cbcefe,caedbe,cccbba,aecddc,EBBEAA,faacaa,fbddef,cfaaef,aedcbd,bbffff,fedabf;040662,240462,520500,404062,100321,662640,242642,151035,051411,442662,442022,200220;426446,224646,466286,828468,866482,484668,504342,224882,486882,824864,668266,444468'";
    mysqli_query($idsconnection_mysqli, $sql,$conn);       
 		$results = mysqli_query($idsconnection_mysqli, $insert_query2,$conn);
 		$results = mysqli_query($idsconnection_mysqli, $insert_query3,$conn);
		$results = mysqli_query($idsconnection_mysqli, $insert_query4,$conn);
		$sql = "INSERT INTO `livehelp_autoinvite` VALUES (1, 'Y', 0, '4', '', 0, '', 'layer','30',0,'N','N','N')";
    mysqli_query($idsconnection_mysqli, $sql,$conn);
