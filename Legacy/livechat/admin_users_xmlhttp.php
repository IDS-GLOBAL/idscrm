...
<?php } ?>
<META HTTP-EQUIV="EXPIRES" CONTENT="Sat, 01 Dec 2016 00:00:00 GMT">
...
if($CSLH_Config['admin_refresh'] == "auto"){  
$html_head .= " setTimeout('csgetimage()', 4000); \n";
$stuffatbottom .= "
<SCRIPT type=\"text/javascript\">
if (ismac > -1) {
	randu=Math.round(Math.random()*9999);
...
<body id="admin_users_xmlhttp" bgcolor=<?php echo $color_background;?>>




<center>
<table id="tbl_admin_users_xmlhttp" bgcolor=<?php echo $color_background;?> cellpadding=0 cellspacing=0 border=0 width=280>
<tr>
...
<br><br>
    ...
    <input type=hidden name="showvisitors" value="<?php echo $UNTRUSTED['showvisitors']; ?>">		
        ...
        <label class="col-sm-2 control-label">How To Load This Window:</label>
        <div class="col-sm-10">
...
</select><a href="javascript:showhelpwindow(1)">(?)</a> <a href="javascript:adminrefresh.submit();"><img src=images/go.gif border=0 width=20 height=20></a>
		</div>
        </div>
</FORM>
   ...
   while($visitor = $chatcheck->fetchRow(DB_FETCHMODE_ASSOC)){
    ...
    if($data_check->numrows() == 0){ 
       ...
       if($counting->numrows() == 0){
            ...
            $actionlink = "<span class='label label-primary ' title='Activate'><a href=admin_users_refresh.php?hidechatters=". $hidechatters . "&ignorelist=$ignorelist&numchats=$numchatsnow&operators=$operators&action=activiate&who=" . $visitor['user_id'] . "&whatchannel=" . $visitor['onchannel'] . "&showvisitors=".$UNTRUSTED['showvisitors']."&needaction=1>".$lang['Activate']."</a></span>\n";            
            if (!(in_array($visitor['user_id'],$ignorelist_array))){
             $actionlink .= "<span class='label label-info ' title='Ignore'><a href=admin_users_refresh.php?hidechatters=". $hidechatters . "&ignorelist=$updated_ignore&numchats=$numchatsnow&operators=$operators&showvisitors=".$UNTRUSTED['showvisitors'].">".$lang['Ignore']."</a></span>\n";
  ...
  $chatcheck = $mydatabase->query($sqlquery);
   while($operatorsarray = $chatcheck->fetchRow(DB_FETCHMODE_ASSOC)){
    ...
    if($foundcommonchannel == 0){
       $chatting = "<img src=img/a8.jpg width=38 height=38 border=0>";
     if($myid != $operatorsarray['user_id'])
        $actionlink = "<span class='label label-warning ' title='begin'><a href=admin_users_refresh.php?hidechatters=". $hidechatters . "&ignorelist=$ignorelist&numchats=$numchatsnow&operators=$operators&action=activiate&who=" . $operatorsarray['user_id'] . "&whatchannel=" . $operatorsarray['onchannel'] . "&conferencein=yes&showvisitors=".$UNTRUSTED['showvisitors']."><font color=007700>". $lang['begin'] . "</font></a></span>\n";
      else {
         $actionlink = ""; 
         $chatting = "<img src=img/a8.jpg width=38 height=38 border=0>\n";
      }            
    } else {
        $chatting = "<table cellpadding=0 cellspacing=0 border=0 bgcolor=".$rowof['txtcolor_alt']."><tr><td><img src=img/a8.jpg width=38 height=38 border=0></td></tr></table>\n";
        if($operatorsarray['user_id'] != $myid)
           $actionlink = "<span class='label label-danger ' title='stop chat'><a href=admin_users_refresh.php?hidechatters=". $hidechatters . "&ignorelist=$ignorelist&numchats=$numchatsnow&operators=$operators&action=leave&who=" . $operatorsarray['user_id'] . "&whatchannel=$foundcommonchannel&clearchannel=N&showvisitors=".$UNTRUSTED['showvisitors'].">".$lang['stop_chat']."</a></span>\n"; 
        }
    
    if($myid != $operatorsarray['user_id'])   
      $html .= "<tr><td width=21><div class='sidebar-message'><div class='pull-left text-center'><img src=img/avatar-profile.png alt='your_pic' class='img-circle message-avatar' width=38 height=38 border=0></div></div></td><td colspan=3><table><tr><td>$chatting</td><td width=90% align=left>  <b>" . $operatorsarray['username'] . "</b>  $actionlink </td></tr></table></td></tr>\n";
    else 
      $lasthtml = "<tr><td width=21><div class='sidebar-message'><div class='pull-left text-center'><img src=img/avatar-profile.png alt='your_pic' class='img-circle message-avatar' width=38 height=38 border=0></div></div></td><td colspan=3><table><tr><td>$chatting</td><td width=90% align=left>  <b>" . $operatorsarray['username'] . "</b>  $actionlink </td></tr></table></td></tr>\n";
    ...
    while($chatters = $sth->fetchRow(DB_FETCHMODE_ASSOC)){
      ...
      if($data_check->numrows() == 0){ 
        // we are not in the department that this user is chatting in.. 
           ...
           $actionlinkouter ="";
           if($counting->numrows() == 0){
             $chatting = "<span class='label label-danger '><i class='fa fa-weixin' aria-hidden='true'></i></span>";
            ...
            $chatting = "<table cellpadding=0 cellspacing=0 border=0 bgcolor=".$rowof['channelcolor']."><tr><td><img src=img/avatar-profile.png alt='your_pic' width=19 height=18 border=0></td></tr></table>\n";
            $actionlink = "";
...
if($UNTRUSTED['showvisitors'] == 1) {
 $html .= "</td><td><b>" .  $lang['visit'] . " <span class='pull-right btn btn-primary  dim'>[onlinenow] ".$lang['online']."</span> <span color=777777>[hiddennow] ".$lang['Hidden']."</span> </td></tr></table>"; 
...
$thediff = $now - $user_info['lastaction'];
 ...
 if($onlinenow > 0){
  $html .= "<table>";
  $html .= "<tr><td><img src=images/arrow_ltr.gif width=38 height=22><select class='form-control' name=whattodo onchange=updatepeople(2)>";
  $html .= "<option value=\"\">".$lang['txt207'].":</option>";
