...
if($UNTRUSTED['typeofview']== "levelvisits"){
  ...
  $bgcolor="F0F1E1"; 
   while($visits = $visits_a->fetchRow(DB_FETCHMODE_ASSOC)){ 
     if($bgcolor==$color_alt2){$bgcolor=$color_alt1; } else { $bgcolor=$color_alt2; }
    print "<tr bgcolor=".$bgcolor."><td><a href=" . $visits['pageurl'] . " target=_blank>" . $visits['pageurl'] . "</a></td><td>" . $visits['directvisits'] . "</td><td>GOT HERE <a href=graph.php?item=" . $visits['recno'] . "&type=refer&typeof=directvisits&whichdepartment=".intval($UNTRUSTED['whichdepartment'])." target=_blank>" . $lang['graph'] . "</a></td></tr>";
   }
  print "</table>";
}
?>
