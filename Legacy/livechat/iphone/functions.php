...
$messages = $mydatabase->query($sqlqueryrun);
	...
	$index=0;	
		while($row = $messages->fetchRow(DB_FETCHMODE_ORDERED)){
            ...
            $channelcolor = (isset($row[8]))? "bgcolor=".$row[8] : "";  
            if($isavisitor){  
            $tablestart = "<table id='chatstart' width=98% cellpadding=0 cellspacing=0 border=0 $channelcolor><tr><td>";
            } else {
            $tablestart = "<table id='chatstart' width=98% cellpadding=0 cellspacing=0 border=0><tr><td width=2><img src=images/blank.gif width=2 height=2></td><td $channelcolor>";            
            }
            $tableend = "</td></tr></table>";
