...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "invoiceBody")) {
  ...
  $updateGoTo = "script_update_invoice.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_queryInvoice =  "SELECT * FROM `idsids_idsdms`.`ids_toinvoices`, `idsids_idsdms`.`dealers` WHERE `ids_toinvoices`.`invoice_dealerid` = `dealers`.`id` 
AND `ids_toinvoices`.`invoice_id` = '$colname_queryInvoice'";
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_lastInvcNum = "SELECT * FROM `idsids_idsdms`.`ids_toinvoices` WHERE `ids_toinvoices`.`invoice_dealerid` = '$ldid' ORDER BY `ids_toinvoices`.`invoice_number` DESC";
...
<title>Billing | Update Invoice</title>
          ...
          <div class="gadget_content">
            ...
            <div class="subblock">
              ...
              <li>
                ...
                <label for="">Select Dealer Company:</label>
                <select name="jumpMenu" id="jumpMenu" onchange="MM_jumpMenu('parent',this,1)">
                  <option value="update-invoice.php?invoice_id=<?php echo $invoiceid; ?>&amp;id=" <?php if (!(strcmp("?id=", $row_queryDealer['id']))) {echo "selected=\"selected\"";} ?>>Select Dealer</option>
                  <?php
do {  
?>
                  <option value="update-invoice.php?invoice_id=<?php echo $invoiceid; ?>&amp;id=<?php echo $row_selectDealer['id']?>"<?php if (!(strcmp($row_selectDealer['id'], $row_queryDealer['id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_selectDealer['company']?></option>
                  <?php
...
?>
              ...
              <li>
                ...
                <input name="invoice_tokenid" class="text" id="invoice_tokenid" value="<?php if(!$row_queryInvoice['invoice_tokenid']){ echo $tkey;}else{ echo $row_queryInvoice['invoice_tokenid']; } ?>" readonly="readonly" />
                <div class="clr"></div>
              </li>
              
              <li>
              
                <label for="invoice_dealerid">Invoice Dealer ID</label>
                <input name="invoice_dealerid" class="text" id="invoice_dealerid" value="<?php if(!$row_queryInvoice['invoice_dealerid']){ echo $did; }else{echo $row_queryInvoice['invoice_dealerid']; } ?>" readonly="readonly" />
                <div class="clr"></div>
              </li>
...
<div class="gadget">
    ...
    <div class="gadget_content">
      <div class="subblock">
        ...
        <p>This Here Is Your Latest Invoices Including Previous Balances, Credits For This <a href="dealer-view-update.php?dealer=<?php if(!$row_queryInvoice['invoice_dealerid']){ echo $did; }else{ echo $row_queryInvoice['invoice_dealerid']; } ?>" title="Click to View Dealer" target="_blank">Dealer</a> | <a href="invoice-print.php?invoice_token=<?php if(!$row_queryInvoice['invoice_dealerid']){ echo $did; }else{echo $row_queryInvoice['invoice_dealerid']; } ?>&id=<?php echo $row_queryInvoice['invoice_id']; ?>" target="_blank">PDF Invoice.</a></p>
        <table width="100%" border="0" cellspacing="3" cellpadding="3" class="gwlines">
          <tr>
            <th>Invoice Number: 
              <input name="invoice_id" type="hidden" id="invoice_id" value="<?php echo $row_queryInvoice['invoice_id']; ?>" /> <input name="invoice_dealerid" type="hidden" id="invoice_dealerid" value="<?php if(!$did){ echo $row_queryInvoice['invoice_dealerid'];}else{ echo $did; } ?>" /></th>
          ...
          <tr>
            ...
            <th><input name="invoice_sendtoclient" type="checkbox" value="<?php echo $row_queryInvoice['invoice_sendtoclient']; ?>" <?php if (!(strcmp($row_queryInvoice['invoice_sendtoclient'],"Y"))) {echo "checked=\"checked\"";} ?> /></th>
            </tr>
...
<!-- gadget right 5 -->
        <div class="gadget">
          <div class="gadget_title gradient37 vertsortable_head">
            <a href="#" class="hidegadget" rel="hide_block"><img src="images/spacer.gif" alt="picture" width="19" height="33" /></a>
            <h3>Update Or Add Items Add Discounts</h3>
...
<?php include("parts/invoice-line-items-update.php"); ?>
  ...
  <tr>
    ...
    <p>You Can't Save This Paid Invoice</p>
    <?php }else{ ?>
