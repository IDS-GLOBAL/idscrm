 ...
 CREATE TABLE IF NOT EXISTS `%table%` (
  ...
  function init(&$c)
  {
    ...
    if (!$db_exists)
    {
      ...
      $result = mysqli_query($idsconnection_mysqli, $query, $db);
      if ($result === FALSE)
      {
  ...
  function setMeta($group, $subgroup, $leaf, $leafvalue = NULL)
  {
    ...
    $res = mysqli_query($idsconnection_mysqli, $sql_count, $db);
    $row = mysql_fetch_array($res, MYSQL_ASSOC);
    if( $row['C'] == 0 )
    {
      mysqli_query($idsconnection_mysqli, $sql_insert, $db);
    ...
    else
    {
      if ($sql_update != "")
      {
        mysqli_query($idsconnection_mysqli, $sql_update, $db);
      }
  ...
  function getMeta($group, $subgroup = null, $leaf = null, $withleafvalue = false)
  {
    ...
    if ($sql_select != "")
    {
      $thisresult = mysqli_query($idsconnection_mysqli, $sql_select, $db);
  ...
  function incMeta($group, $subgroup, $leaf)
  {
    ...
    else
    {
      ...
      mysqli_query($idsconnection_mysqli, $sql_update, $db);
      $res = mysqli_query($idsconnection_mysqli, 'SELECT LAST_INSERT_ID();', $db);      
      $row = mysql_fetch_array($res, MYSQL_ASSOC);
  ...
  function rmMeta($group, $subgroup = null, $leaf = null)
  {
    ...
    mysqli_query($idsconnection_mysqli, $sql_delete, $db);
    return true;
  }
