  ...
  function file_put_contents($filename, $content, $flags = null, $resource_context = null)
    {
      ...
      if (($bytes = @fwrite($fh, $content)) === false) {
        $errormsg =  'file_put_contents() Failed to write %d bytes to %s ['.$filename.']',
                            $length,
                            $filename);
      ...
      if ($bytes != $length) {
        $errormsg =  'file_put_contents() Only %d of %d bytes written, possibly out of free disk space. ['.$filename.']',
                            $bytes,
                            $length);
        user_error($errormsg, E_USER_WARNING);
        return false;
