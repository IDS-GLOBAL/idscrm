...
class PDF417 {
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		if (!empty($macro)) {
			...
			$macrocw[] = 928;
			// segment index
			$cw = $this->getCompaction(902,  '%05d', $macro['segment_index']), false);
			...
			foreach ($optmodes as $k => $omode) {
				if (isset($macro['option_'.$k])) {
					...
					$macrocw[] = $k;
					if ($optsize[$k] == 2) {
						$macro['option_'.$k] =  '%05d', $macro['option_'.$k]);
					} elseif ($optsize[$k] == 4) {
						$macro['option_'.$k] =  '%010d', $macro['option_'.$k]);
					}
		...
		for ($r = 0; $r < $rows; ++$r) {
			...
			// left row indicator
			$row .=  '%17b', $this->clusters[$cid][$L]);
			// for each column
			for ($c = 0; $c < $cols; ++$c) {
				$row .=  '%17b', $this->clusters[$cid][$codewords[$k]]);
				++$k;
			}
			switch ($cid) {
				case 0: {
