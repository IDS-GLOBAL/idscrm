...
$query_dealer_url = "SELECT * FROM `idsids_idsdms`.dealers WHERE id = '94'";
...
if($row_dealer_url['id']){$did=$row_dealer_url['id'];}else{$did=94;}
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_manager_nav = "SELECT * FROM manager_person WHERE dealer_id = '$did' ORDER BY manager_id ASC";
...
mysqli_select_db($idsconnection_mysqli, $database_idsconnection);
$query_find_acct_rep =  "SELECT * FROM account_person WHERE dealer_id = '$did' AND account_person_id = %s", GetSQLValueString($colname_find_acct_rep, "int"));
...
*/
if(isset($_GET['token'])){
@$gettoken = mysql_real_escape_string($_GET['token']);
}
...
//$total_avilable = formatMoney($total_avilable , true); 
	...
	$user=NULL;
 	...
 	//require_once("db_facebook.php"); 

 	include_once("../Libary/sessioncookies.php"); 
